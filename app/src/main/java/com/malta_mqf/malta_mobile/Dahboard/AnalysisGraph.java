...
import com.malta_mqf.malta_mobile.Model.oldSalesReturnsForTab;
import com.malta_mqf.malta_mobile.Model.salesReturnsDateWise;
import com.malta_mqf.malta_mobile.R;
import com.malta_mqf.malta_mobile.Utilities.ALodingDialog;
import com.malta_mqf.malta_mobile.dashboard_analysis;
...
public class AnalysisGraph extends BaseActivity {
    ...
    TextView BTGText,DailyAverageSales,mtdTextView,ytdSalesTextView,tomorrowsalesmustbeText,targetTextView,btgTextView,targetSalesText,targetsalestypeText,achievementpercentageText,ForcastTextView,DailyShortFallText;
...
//        }
    ...
    private void loadYearlyAndMonthlyDateRanges() {
        ...
        // loadTotalGrossAmountOnline(startOfMonth, endDate, vanId, outletId);
        // for monthly
        String currentDate = new SimpleDateFormat("yyyy-MM", Locale.getDefault()).format(new Date());
        System.out.println("Current Date: " + currentDate); // Example output: 22-07
       

    }

    private void currentMonthAndYearToDate(String startOfMonth, String endDate, String startOfYear, String endDate1, String vanId) {
        // Monthly Sales
        String monthUrl = ApiLinks.oldSalesAndReturns + "?from_date=" + startOfMonth + "&to_date=" + endDate + "&van_id=" + vanId;
        Call<oldDashBoardResponse> monthlyCall = apiInterface.getoldDashBoardData(monthUrl);

        monthlyCall.enqueue(new Callback<oldDashBoardResponse>() {
            @Override
            public void onResponse(Call<oldDashBoardResponse> call, Response<oldDashBoardResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    oldSalesReturnsForTab salesData = response.body().getOldSalesReturnsForTabList();

                        salesTotalNetMonthly = Double.parseDouble(salesData.getActualSales());
            ...
            @Override
            public void onFailure(Call<oldDashBoardResponse> call, Throwable t) {
                System.out.println("Monthly API call failed: " + t.getMessage());
            }

        });

        String yearUrl = ApiLinks.oldSalesAndReturns + "?from_date=" + startOfYear + "&to_date=" + endDate1 + "&van_id=" + vanId;
        System.out.println("yearUrl is "+yearUrl);
        Call<oldDashBoardResponse> yearlyCall = apiInterface.getoldDashBoardData(yearUrl);

        yearlyCall.enqueue(new Callback<oldDashBoardResponse>() {
            @Override
            public void onResponse(Call<oldDashBoardResponse> call, Response<oldDashBoardResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    oldSalesReturnsForTab salesData = response.body().getOldSalesReturnsForTabList();
...
//    }
   ...
   /* @RequiresApi(api = Build.VERSION_CODES.O)
    ...
    private void loadTotalGrossAmountForDateRangeOffline(String fromDate, String toDate, String outletId) {
        ...
        if (salesMap.isEmpty() && returnsMap.isEmpty() && reusableReturnsMap.isEmpty()) {
            Toast.makeText(this, "No data found for the selected date range and outlet.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        double totalReturns = returnsDB.getTotalReturnAmountByDate(fromDate + " 00:00:00", toDate + " 23:59:59", outletId) - reusableAmount;
        System.out.println("totalReturns :" + totalReturns);
        String formatted = String.format("%.2f", totalReturns);
        double formattedtotalReturns = Double.parseDouble(formatted);
        System.out.println("formattedtotalReturns :" + formattedtotalReturns);
        ...
        double salesPercent = calculateSalesPercentage(totalSales, totalReturns);
        double returnPercent = calculateReturnPercentage(totalSales, totalReturns);
        String formattedpercentage = String.format("%.2f", returnPercent);
        double formattedreturnPercent = Double.parseDouble(formattedpercentage);
        System.out.println("formattedreturnPercent :" + formattedreturnPercent);


        updateTextViews(totalSales, formattedtotalReturns, salesPercent, formattedreturnPercent);
    }
    ...
    private void loadTotalGrossAmountOnline(String fromDate, String toDate, String vanId, String outletId) {
        String url = ApiLinks.oldSalesAndReturns + "?from_date=" + fromDate + "&to_date=" + toDate + "&van_id=" + vanId;
        ...
        Call<oldDashBoardResponse> getDetails = apiInterface.getoldDashBoardData(url);
        Call<SalesReturnsForTabList> getDetailsDateWise = apiInterface.getDashBoardGraphData(url2);

        // First API call for dashboard data
        getDetails.enqueue(new Callback<oldDashBoardResponse>() {
            @Override
            public void onResponse(Call<oldDashBoardResponse> call, Response<oldDashBoardResponse> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    oldDashBoardResponse dashBoardData = response.body();


                        oldSalesReturnsForTab salesData = dashBoardData.getOldSalesReturnsForTabList();

                        try {
                            double salesTotalNet = isValidDouble(salesData.getActualSales())
                                    ? Double.parseDouble(salesData.getActualSales()) : 0.0;

                            double returnsTotalNet = isValidDouble(salesData.getActualReturns())
                                    ? Double.parseDouble(salesData.getActualReturns()) : 0.0;

                            double salespercentage = isValidDouble(salesData.getSalesPercentage())
                                    ? Double.parseDouble(salesData.getSalesPercentage()) : 0.0;

                            double returnPercent = isValidDouble(salesData.getReturnPercentage())
                                    ? Double.parseDouble(salesData.getReturnPercentage()) : 0.0;
                            ...
                            updateTextViews(salesTotalNet, returnsTotalNet, salespercentage, returnPercent);
    ...
    private void updateTextViews(Double totalSales,Double totalReturns,Double netsales,Double returnPercent) {
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent intent = new Intent(AnalysisGraph.this, dashboard_analysis.class);
